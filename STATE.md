...
- v2(进行中,路线B):目标=让引擎自动吐出结构化数据(装配器),打通"引擎→data_base.json→网站"全自动+人工放行。
  ...
  - 进行中(2026-07-02):④Netlify v03=netlify.toml已入库(publish=site;ignore=未改site/跳过构建,含首建防误跳)。发布模式已裁决A=锁定发布(Deploys页"Lock to stop auto publishing"):push照常构建为待发布版本,老板点"Publish deploy"才上线=人工放行落在Netlify层,git流程零改动。待老板网页端:导入GitHub仓库→站名bubbleradarv03→首次部署验收(指纹=页面观测区显示新值DXY 101.23/VVIX 89.04/SOX 13353.28)→开启锁定。老站bubbleradar0622暂留,v03验收后退役。已登记小瑕疵:yfinance对交易中的当日返回盘中bar(如DXY数据日07-02为盘中值),"只取已收盘bar"过滤留⑤一并处理。
  - 待做:⑤抓取器并入GitHub Actions日跑(留云端回写轮) ⑥核心11指标自动更新=series.pkl追加保锚点+AAII录入派生(最难,专门一轮)。
