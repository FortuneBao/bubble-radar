...
- v2(进行中,路线B):目标=让引擎自动吐出结构化数据(装配器),打通"引擎→data_base.json→网站"全自动+人工放行。
  ...
  - 待做:③观测抓取器(FRED DGS2/DGS10 CSV/API + yfinance DX-Y.NYB/^VVIX/^SOX,自动写observations.json,抓取失败置null;AAII手动周更输入Bull/Neutral/Bear三原始数,派生流程另行) ④Netlify新站点bubbleradarv03连GitHub盯site/自动部署。
